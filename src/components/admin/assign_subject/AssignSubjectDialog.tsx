...
const AssignSubjectDialog = ({ open, onOpenChange, getAssignSubject, assignedSubjects }: AssignSubjectDialogProps) => {
  ...
  const handleSelectTeacher = (teacher: UserInfo) => {
    ...
    if (assignedSubjects) {
      const assignedSubjectsForTeacher = assignedSubjects.data
        .filter((as) => as.teacher.id === teacher.id)
        .map((as) => ({
          id: as.subject.id,
        ...
        <div className="space-y-5">
          <div className="space-y-2">
            ...
            >
              <PopoverTrigger className="w-full">
                <Button variant="outline" disabled={isLoadingTeachers} className="w-full justify-between">
                  {selectedTeacher ? `${selectedTeacher.code} - ${selectedTeacher.name}` : 'Chọn giảng viên'}
              ...
              <PopoverContent className="min-w-[450px] p-2 space-y-2">
                <div className="relative flex-1">
                  ...
                  <Input type="text" placeholder="Tìm kiếm theo tên, mã giảng viên..." value={searchTeacher} onChange={(e) => setSearchTeacher(e.target.value)} className="pl-10" />
                </div>
          ...
          <div className="space-y-2">
            ...
            {selectedTeacher ? (
              ...
              >
                <PopoverTrigger className="w-full">
                  <Button variant="outline" className="w-full justify-between" disabled={isLoadingSubjects || !selectedTeacher}>
                    {selectedSubjects.length > 0 ? `${selectedSubjects.length} môn học được chọn` : 'Chọn môn học'}
                    <svg className="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                    </svg>
                  </Button>
                ...
                <PopoverContent className="min-w-[450px] p-2 space-y-2">
                  <div className="relative flex-1">
                    <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-4 w-4" />
                    <Input type="text" placeholder="Tìm kiếm theo tên, mã môn học..." value={searchSubject} onChange={(e) => setSearchSubject(e.target.value)} className="pl-10" />
                  </div>
                  <div className="max-h-[300px] overflow-y-auto">
                    {isLoadingSubjects ? (
                      <div className="flex items-center justify-center h-full p-3">
                      ...
                      <ScrollArea className="h-40 w-full">
                        {subjects?.data && subjects.data.length > 0 ? (
                          subjects.data.map((subject) => (
                            <div key={subject.id} className="p-2 hover:bg-gray-100 cursor-pointer rounded-md flex items-center" onClick={() => handleToggleSubject(subject)}>
                              <input type="checkbox" checked={selectedSubjects.some((s) => s.code === subject.code)} onChange={() => handleToggleSubject(subject)} className="mr-2" />
        ...
        <DialogFooter>
          <div className="flex justify-between space-x-2">
            ...
            <div className="flex space-x-2">
              <Button type="button" className="bg-primary hover:bg-primary/90 cursor-pointer" onClick={handleAssignTeacher} disabled={!selectedSubjects || isLoading}>
