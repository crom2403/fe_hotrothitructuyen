      ...
      <ScrollArea className="h-[70vh] pr-2">
        <div className="space-y-4 text-sm text-gray-700">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <span className="font-semibold">Độ khó:</span> <Badge className={getDifficultyColor(question.difficulty_level.name)}>{question.difficulty_level.name}</Badge>
            </div>
            <div>
              <span className="font-semibold">Người tạo:</span> {question.created_by.full_name}
            </div>
            <div>
              <span className="font-semibold">Trạng thái duyệt:</span>{' '}
              <Badge variant="outline" className={getStatusColor(question.review_status)}>
                {getStatusText(question.review_status)}
              </Badge>
            </div>
            <div>
              <span className="font-semibold">Ngày tạo:</span> {new Date(question.created_at).toLocaleString('vi-VN')}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mt-2 p-3 border rounded-md bg-gray-50">{parse(question.content)}</div>
          </div>
          <div className="space-y-2">
            <p className="font-semibold">Các phương án:</p>
            {question.answers.map((answer: any, index) => (
              <div key={answer.id} className={`p-3 border rounded-md ${answer.is_correct ? 'bg-green-50 border-green-500' : 'bg-white'}`}>
                <div className="flex justify-between items-center">
                  <span className="font-medium">Đáp án {index + 1}:</span>
                  {answer.is_correct && <Badge className="bg-green-600 text-white">Đúng</Badge>}
                </div>
