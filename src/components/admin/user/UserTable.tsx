      ...
      <CardContent>
        <div className="flex items-center space-x-4 mb-4">
          <div className="relative flex-1">
            ...
            <Input placeholder="Tìm kiếm theo tên, mã số hoặc email..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10" />
          </div>
            ...
            {isLoading ? (
              ...
              users.map((user) => (
                <TableRow key={user.code}>
                  ...
                  <TableCell>
                    <Badge className={cn(user.is_active ? 'bg-green-500' : 'bg-red-500')}>{user.is_active ? 'Hoạt động' : 'Bị khóa'}</Badge>
                  </TableCell>
                  <TableCell>
                    <DropdownMenu>
                      <DropdownMenuTrigger>
