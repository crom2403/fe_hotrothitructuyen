    ...
    <div className="flex flex-col w-full h-screen items-center justify-center gap-7 bg-gray-50">
      <div className="flex flex-col items-center justify-center gap-2 bg-white rounded-lg p-5 shadow-lg">
        <div className="flex flex-col items-center justify-center gap-2 w-[320px]">
          ...
          <p className="text-2xl font-bold mt-4 text-center">Quên mật khẩu?</p>
          <p className="text-sm text-gray-700 text-center">Nhập email đã được liên kết với tài khoản để lấy lại mật khẩu</p>
        </div>
        <div>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5" action="POST">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <div>
                        <Input type="email" placeholder="Nhập email" {...field} className="w-[320px] h-[40px] border-2 border-gray-300 rounded-md" />
                      </div>
                    </FormControl>
                    <FormMessage />
                  </FormItem>
              ...
              />
              <Button type="submit" className="w-[320px] h-[40px] bg-blue-800 hover:bg-blue-700 text-white rounded-md hover:opacity-90" disabled={isLoading}>
                {isLoading ? 'Đang gửi...' : 'Gửi'}
              </Button>
            </form>
          </Form>
        </div>
        <div className="flex items-center justify-center gap-2 hover:cursor-pointer hover:underline hover:text-blue-500" onClick={() => navigate(path.LOGIN)}>
          <ChevronLeft size={20} />
