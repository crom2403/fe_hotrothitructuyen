      ...
      <DialogContent className="space-y-5">
        ...
        <div className="space-y-5">
          <div className="space-y-2">
            ...
            >
              <PopoverTrigger className="w-full">
                <Button variant="outline" className="w-full justify-between" disabled={isLoadingStudyGroup}>
                  {selectedStudyGroup ? `${selectedStudyGroup.study_group_code} - ${selectedStudyGroup.study_group_name}` : 'Chọn lớp học phần'}
              ...
              <PopoverContent className="min-w-[450px] p-2">
                <div className="relative flex-1">
                  ...
                  <Input type="text" placeholder="Tìm kiếm theo tên, mã lớp hoặc giáo viên..." value={searchStudyGroup} onChange={(e) => setSearchStudyGroup(e.target.value)} className="pl-10" />
                </div>
                {isLoadingStudyGroup ? (
                  ...
                  <ScrollArea className="h-40 w-full">
                    {studyGroup?.data && studyGroup.data.length > 0 ? (
                      studyGroup.data.map((group) => (
                        <div key={group.study_group_id} className="p-2 hover:bg-gray-100 cursor-pointer rounded-md" onClick={() => handleSelectStudyGroup(group)}>
          ...
          <div className="space-y-2">
            ...
            <Popover
              open={openStudent && selectedStudyGroup !== null}
              onOpenChange={(open) => {
                setOpenStudent(open);
              }}
            >
              <PopoverTrigger className="w-full">
                <Button variant="outline" className="w-full justify-between" disabled={isLoadingStudent || !selectedStudyGroup}>
                  {selectedStudents.length > 0 ? `${selectedStudents.length} sinh viên được chọn` : 'Chọn sinh viên'}
                  <svg className="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                  </svg>
                </Button>
              ...
              <PopoverContent className="min-w-[450px] p-2">
                <div className="relative flex-1">
                  <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-4 w-4" />
                  <Input type="text" placeholder="Tìm kiếm theo tên, mã hoặc email..." value={searchStudent} onChange={(e) => setSearchStudent(e.target.value)} className="pl-10" />
                </div>
                {isLoadingStudent ? (
                  <div className="flex justify-center">
                    <Loader2 className="h-6 w-6 animate-spin text-gray-500 mt-2" />
                  ...
                  <ScrollArea className="h-40 w-full">
                    {student?.data && student.data.length > 0 ? (
                      student.data.map((user) => (
                        <div key={user.id} className="p-2 hover:bg-gray-100 cursor-pointer rounded-md flex items-center" onClick={() => handleToggleStudent(user)}>
                          <input type="checkbox" checked={selectedStudents.some((s) => s.code === user.code)} onChange={() => handleToggleStudent(user)} className="mr-2" />
                          {`${user.code} - ${user.name}`}
                        </div>
                      ))
                    ) : (
        ...
        <div className="flex justify-between space-x-2">
          ...
          <div className="flex space-x-2">
            ...
            <Button type="button" className="bg-primary hover:bg-primary/90 cursor-pointer" onClick={handleAddStudent} disabled={!selectedStudyGroup || isLoadingAddStudent}>
