    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="min-w-5xl p-6">
        <DialogHeader>
          ...
          <DialogDescription>
            Dưới đây là các đề thi đã được tạo theo nhóm học và môn học.
          </DialogDescription>
        </DialogHeader>
                ...
                <TableBody>
                  {exams?.map((exam) => (
                    <TableRow key={exam.id}>
                      ...
                      <TableCell>
                        {format(new Date(exam.start_time), 'HH:mm dd/MM')} -{' '}
                        {format(new Date(exam.end_time), 'HH:mm dd/MM')}<br />
                      ...
                      <TableCell>
                        <Badge
                          variant={
                            exam.opening_status === 'open'
                              ? 'default'
                              : exam.opening_status === 'finished'
                                ? 'destructive'
                                : 'secondary'
                          }
                        >
                      ...
                      <TableCell>
                        <Badge variant="outline">
                          {exam.test_type === 'midterm'
                            ? 'Giữa kỳ'
                            : exam.test_type === 'exercise'
                              ? 'Bài tập'
                              : 'Khác'}
                        </Badge>
                      </TableCell>
