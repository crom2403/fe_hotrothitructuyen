...
const ExamResultDetail = () => {
  ...
  const filteredResults = examResult.filter((result) => {
    ...
    const isSearchMatch =
      result.student_code.toLowerCase().includes(searchLower) ||
      result.student_full_name.toLowerCase().includes(searchLower)

    const isStatusMatch = statusFilter === 'all' || (
      (statusFilter === 'completed' && result.exam_attempt_submitted_at !== null) ||
      (statusFilter === 'pending' && result.exam_attempt_submitted_at === null)
    )
  ...
  const averageScore = examResult.length > 0
    ? (examResult.reduce((sum, r) => sum + (r.exam_attempt_score || 0), 0) / examResult.length).toFixed(2)
    : 0
  const passRate = examResult.length > 0
    ? ((examResult.filter(r => (r.exam_attempt_score || 0) >= 5).length / examResult.length) * 100).toFixed(0)
    : 0 // sửa lại theo pass point
                      ...
                      {filteredResults.map((result) => (
                        <TableRow key={result.exam_attempt_id}>
                          ...
                          <TableCell className="text-center font-semibold">
                            {result.exam_attempt_score > 0 ? result.exam_attempt_score : "-"}
                          </TableCell>
                          <TableCell className="text-center">{getScoreBadge(result.exam_attempt_score)}</TableCell>
                          <TableCell className="text-center">{result.exam_attempt_duration_seconds}</TableCell>
                          <TableCell className="text-center">{formatDate(result.exam_attempt_submitted_at, "dd/MM/yyyy HH:mm:ss")}</TableCell>
                          <TableCell className="text-center">
                            <Badge variant={result.exam_attempt_submitted_at === null ? "secondary" : "default"}>
                              {result.exam_attempt_submitted_at === null ? "Chưa thi" : "Đã thi"}
                            </Badge>
                          </TableCell>
                          <TableCell>
                            <DropdownMenu>
