...
function VideoPopupQuestion({ question, answers, onAnswerChange }: { question: Question; answers: Record<number, string>; onAnswerChange: (timeIndex: number, answer: string) => void }) {
  ...
  useEffect(() => {
    ...
    const handleTimeUpdate = () => {
      ...
      mockVideoPopupConfig.popup_times.forEach((popup, index) => {
    ...
    <div className="space-y-4">
      <div className="relative bg-black rounded-lg overflow-hidden shadow-2xl">
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/60 to-transparent p-4">
          <div className="relative mb-4">
            ...
            {mockVideoPopupConfig.popup_times.map((popup, index) => {
          ...
          <div className="flex items-center justify-between text-white">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="text-sm bg-black/50 px-3 py-1 rounded-full">
                ...
                <span className="text-gray-400">/{mockVideoPopupConfig.popup_times.length}</span>
        ...
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Câu hỏi video - {showQuestion !== null ? formatTime(mockVideoPopupConfig.popup_times[showQuestion]?.time || 0) : ''}
            </DialogTitle>
          ...
          {showQuestion !== null && (
            <div className="space-y-4">
              <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                <p className="font-medium text-blue-900">{mockVideoPopupConfig.popup_times[showQuestion].question}</p>
              </div>
              <RadioGroup onValueChange={handleQuestionAnswer}>
                {mockVideoPopupConfig.popup_times[showQuestion].options.map((option, index) => (
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <Badge variant={Object.keys(answers).length === mockVideoPopupConfig.popup_times.length ? 'default' : 'secondary'}>
            {Object.keys(answers).length}/{mockVideoPopupConfig.popup_times.length} hoàn thành
          </Badge>
        </div>
