...
const ExamResults = () => {
  ...
  const averageScore = examResults.reduce((sum, result) => sum + (result.score || 0), 0) / examResults.length || 0;
  ...
  const handleGetExamResults = async () => {
    ...
    try {
      ...
      console.log('Exam Results:', response.data); // Kiểm tra dữ liệu
    } catch (error) {
                ...
                <div key={result.id} className="border rounded-lg p-4 hover:shadow-md transition-shadow flex flex-col justify-between h-full">
                  <div>
                    <div className="flex items-start justify-between mb-4">
                      <div className="space-y-1">
                        <h3 className="text-lg font-semibold line-clamp-1">{result.exam_name}</h3>
                        <p className="text-sm text-gray-600 line-clamp-1">{result.exam_subject}</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <Badge className={`${getExamTypeColor(result.exam_type)}`}>{getExamType(result.exam_type)}</Badge>
                        <Badge className={getStudentGradeColor(result.exam_pass_point, result.score || 0)}>
                          {result.score > result.exam_pass_point ? 'Đậu' : 'Rớt'}
                        </Badge>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                      <div className="space-y-2">
                        ...
                        <p className={`text-2xl font-bold ${getPerformanceColor(Number((result.score || 0).toFixed(2)))}`}>
                          {result.exam_allow_review_point ? (result.score?.toFixed(2) || 'Chưa có') : 'Không hiển thị'}
                        </p>
                      </div>
                      <div className="space-y-2">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-sm">{handleSecondToMinute(result.duration_seconds)}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2 mb-4">
                      <div className="flex justify-between text-sm">
                        ...
                        <span>{result.submitted_at ? '100%' : '0%'}</span>
                      </div>
                      <Progress value={result.submitted_at ? 100 : 0} className="h-2" />
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-auto">
                    ...
                    {result.exam_allow_review && (
                      <Button variant="outline" size="sm" onClick={() => handleViewExam(result.id)}>
